...
I wired [Mermaid.js](https://mermaid.js.org/) into this blog so I can drop
diagrams straight into a post. It's lazy-loaded and client-rendered, so the
initial page stays light and I get an architecture sketch or state machine
...
<MermaidDiagram
  height={360}
  className="mermaid-featured-card mermaid-featured-temperature"
  diagram={`xychart-beta
	  accTitle: Weekly Temperature Chart
	  accDescr: This chart shows high and low temperatures across a week, with bars for daily highs and a line for daily lows.
	  title "Temperature by day of week"
	  x-axis ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
	  y-axis "Degrees F" 70 --> 90
	  bar [74, 76, 79, 83, 88, 90, 85]
	  line [71, 72, 74, 76, 79, 78, 75]`}
/>
...
Generic tasks sorted by impact and effort:

<MermaidDiagram
  height={420}
  className="mermaid-featured-card mermaid-example-quadrant"
  useLazyLoading
  diagram={`quadrantChart
	  accTitle: Task Priority Matrix
	  accDescr: This quadrant chart organizes generic tasks by effort and impact, separating quick wins, big bets, fill-ins, and work to reconsider.
	  ...
	  Update docs: [0.22, 0.72]
	  Prototype UI: [0.62, 0.78]
	  Launch campaign: [0.82, 0.86]
	  Clean inbox: [0.18, 0.32]
	  Rename files: [0.34, 0.24]
	  Rewrite module: [0.78, 0.44]`}
/>
...
<MermaidDiagram
  ...
  diagram={`pie title Technology Stack
	  ...
	  accDescr: This pie chart shows the rough distribution of technologies powering this site: TanStack Start 35%, TypeScript 25%, Tailwind CSS 20%, MDX with Shiki 15%, and Tooling 5%.
	  "TanStack Start" : 35
	  "TypeScript" : 25
	  "Tailwind CSS" : 20
	  "MDX + Shiki" : 15
...
Code fences get syntax highlighting at build time via
[Shiki](https://shiki.style/) — no runtime JS, no theme flash. Same custom
`Idle Toes` theme every post.
