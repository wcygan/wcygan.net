...
description: Power tools I rely on
...
I use `just` for task orchestration across my projects (similar to `make`). For example, I would have commands like `just up`, `just down`, `just test`, etc...
...
Seems like a nice successor to git. But I don't think there is a TUI as nice as LazyGit yet, so I stay on git.
